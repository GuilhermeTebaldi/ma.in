  ...
  return (
    <section id="progetti" className="relative py-28 bg-gradient-to-b from-[#0a0c10] via-[#0d1117] to-[#0a0c10] overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-6 md:px-10">
        ...
        {/* MOBILE: empilhado. DESKTOP: duas colunas lado a lado, alternando a diagonal */}
        <div className="grid gap-10 md:gap-12 md:grid-cols-2">
          {list.map((p, i) => {
            const right = i % 2 === 1; // alterna orientação
            const sk = right ? '4deg' : '-4deg';
            return (
              <article
                key={i}
                data-i={i}
                ...
                className={`relative group gpu transition-all duration-700 ease-out will-change-transform
                  ${visible[i] ? 'opacity-100 translate-x-0' : (right ? 'opacity-0 translate-x-10 md:translate-x-12' : 'opacity-0 -translate-x-10 md:-translate-x-12')}`}
                style={{ ['--sk']: sk }}
              >
                <div className="relative">
                  ...
                  {/* Título e meta flutuando conforme a orientação */}
                  <div className={`absolute ${right ? 'right-6 md:right-4' : 'left-6 md:left-4'} bottom-6 -skew-y-[var(--sk)]`}>
                    <h3 className="text-lg md:text-xl lg:text-2xl font-semibold text-white drop-shadow">{p.t}</h3>
                    <p className="text-xs md:text-sm text-white/75">Engineering & Design</p>
                  </div>
                </div>
              </article>
            );
        ...
        <div className="mt-16 text-center">
          <a href="#contatti" className="inline-block px-10 py-4 rounded-full border border-sky-400 text-sky-400 hover:bg-sky-400 hover:text-white font-bold text-sm uppercase tracking-wider transition-colors shadow-[0_0_15px_rgba(14,165,233,0.25)]">Scopri di più</a>
        </div>
      </div>
    </section>
  );
